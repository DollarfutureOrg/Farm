...
import { Image, Heading, RowType, Toggle, Text } from 'dlf-ui-kit'
import styled from 'styled-components'
import { BLOCKS_PER_YEAR, DLF_PER_BLOCK, DLF_POOL_PID } from 'config'
...
const Farms: React.FC = () => {
  ...
  const cakePrice = usePriceEthBusd()
  ...
  const farmsList = useCallback(
    (farmsToDisplay): FarmWithStakedValue[] => {
      const cakePriceVsBNB = new BigNumber(farmsLP.find((farm) => farm.pid === DLF_POOL_PID)?.tokenPriceVsQuote || 0)
      let farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        if (!farm.tokenAmount || !farm.lpTotalInQuoteToken || !farm.lpTotalInQuoteToken) {
          return farm
        }
        const cakeRewardPerBlock = DLF_PER_BLOCK.times(farm.poolWeight)
        ...
        } else if (farm.quoteTokenSymbol === QuoteToken.DLF) {
        ...
        if (farm.quoteTokenSymbol === QuoteToken.DLF) {
  ...
  const rowData = farmsStaked.map((farm) => {
    ...
    const lpLabel = farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('PANDLF', '')
    ...
    <>
      <Header>
        ...
        <Heading size="lg" color="text">
          {TranslateString(999, 'Stake Lion Food (LF) tokens to earn.')}
        </Heading>
      </Header>
      <Page>
        <ControlContainer>
        ...
        {renderContent()}
        <StyledImage src="https://i.ibb.co/zs58rgZ/3-removebg-preview.png" alt="Pancake illustration" width={120} height={103} />
      </Page>
    </>
  )
}
