...
import { Button, useModal, IconButton, AddIcon, MinusIcon } from 'dlf-ui-kit'
...
const Staked: React.FunctionComponent<FarmWithStakedValue> = ({
  ...
  if (!account) {
    return (
      <ActionContainer>
        <ActionTitles>
          <Subtle>{TranslateString(999, 'START DLFMING')}</Subtle>
        </ActionTitles>
        <ActionContent>
  ...
  return (
    <ActionContainer>
      <ActionTitles>
        <Subtle>{TranslateString(999, 'ENABLE DLFM')}</Subtle>
      </ActionTitles>
      <ActionContent>
